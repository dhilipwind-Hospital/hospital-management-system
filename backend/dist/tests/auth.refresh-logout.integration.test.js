...
const refreshRepo = {
    create: jest.fn((data) => ({ id: `rt_${Math.random().toString(36).slice(2)}`, isRevoked: false, revokedAt: null, revokedByIp: null, replacedByToken: null, ...data })),
...
describe('Auth integration - refresh token', () => {
    it('issues new tokens when provided a valid refresh token', async () => {
        ...
        const loginRes = await (0, supertest_1.default)(app).post('/api/auth/login').send({ email: 'rif@example.com', password: 'StrongP@ss1' });
        expect(loginRes.status).toBe(200);
        const prevRefresh = (_a = loginRes.body) === null || _a === void 0 ? void 0 : _a.refreshToken;
        expect(prevRefresh).toBeTruthy();
        const refreshRes = await (0, supertest_1.default)(app).post('/api/auth/refresh-token').send({ refreshToken: prevRefresh });
        expect(refreshRes.status).toBe(200);
        expect((_b = refreshRes.body) === null || _b === void 0 ? void 0 : _b.accessToken).toBeTruthy();
        const newRefresh = (_c = refreshRes.body) === null || _c === void 0 ? void 0 : _c.refreshToken;
    ...
    it('rejects missing refresh token', async () => {
        const app = buildApp();
        const res = await (0, supertest_1.default)(app).post('/api/auth/refresh-token').send({});
...
describe('Auth integration - logout', () => {
    it('deletes the stored refresh token and clears cookie', async () => {
        ...
        await u.hashPassword();
        await userRepo.save(u);
        const loginRes = await (0, supertest_1.default)(app).post('/api/auth/login').send({ email: 'logout@example.com', password: 'StrongP@ss1' });
        expect(loginRes.status).toBe(200);
        const refreshToken = (_a = loginRes.body) === null || _a === void 0 ? void 0 : _a.refreshToken;
        // Call logout with refresh token in body
        const logoutRes = await (0, supertest_1.default)(app).post('/api/auth/logout').send({ refreshToken });
